...
### 🚀 Added
- **OpenClaw API Integration**: Pre-configured Gateway URL (`http://localhost:9876`) with OAuth endpoints
- **Connection Test**: Added "Test Connection" button in settings to verify Gateway connectivity
- **Chinese Tutorial**: Complete `TUTORIAL_ZH.md` with installation, setup, and troubleshooting guide

### 🔧 Fixed
- **EPIPE Startup Error**: Fixed PTY Host pipe errors on Windows with improved error handling
- **PTY Host Stability**: Added uncaught exception handlers and graceful degradation
- **Windows Compatibility**: Enhanced conpty backend support for better Windows terminal experience

### 📚 Documentation
- Added comprehensive Chinese quick start guide
- Updated security fix documentation
...
### 🚀 Added
- **OpenClaw API Configuration UI**: New settings panel for configuring OpenClaw Gateway connection (URL, OAuth credentials, connection testing)
